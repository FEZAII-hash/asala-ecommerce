...
const categories = ["Caftan", "Takchita", "Jebba", "Robe", "CÉRÉMONIE", "Accessoire"];
...
const CollectionPage: React.FC = () => {
  ...
  const [sort, setSort] = useState(
    searchParams.get("filter") === "new" ? "newest" : "relevance"
  );
  ...
  const listedProducts = useMemo(() => {
    const result = products.filter((product) => {
      ...
      if (searchParams.get("filter") === "new" && !product.new) {
        return false;
      }
      return true;
    });
    ...
    return result;
  }, [category, size, color, maxPrice, sort, searchParams]);
        ...
        <div className="asala-container pt-8 pb-12 lg:pt-10 lg:pb-16">
          ...
          <div className="max-w-2xl">
            ...
            <p className="mt-3 sm:mt-4 text-[12px] sm:text-[14px] leading-relaxed text-stone max-w-xl">
              Des pièces d'exception pensées pour accompagner les instants qui comptent : caftans brodés d'or, takchitas d'apparat, jebbas contemporaines et parures artisanales.
            </p>
          </div>
        </div>
      </section>
